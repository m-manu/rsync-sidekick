...
func TestRSyncSidekick(t *testing.T) {
	...
	actions1, syncErr1 := getSyncActionsWithProgress(runID, srcPath, exclusionsForTests, dstPath, true, 2*time.Second, false)
	...
	rsErr1 := rsyncSidekick(runID, srcPath, exclusionsForTests, dstPath, "", false, false, 2*time.Second, false)
	...
	actions2, syncErr2 := getSyncActionsWithProgress(runID, srcPath, exclusionsForTests, dstPath, false, 2*time.Second, false)
	stopIfError(t, syncErr2)
	assert.Equal(t, []action.SyncAction{}, actions2)
	deleteFile(atSrc("/another_code/sort.go.txt"))
	actions3, syncErr3 := getSyncActionsWithProgress(runID, srcPath, exclusionsForTests, dstPath, true, 2*time.Second, false)
...
func TestTimestampPropagationBug(t *testing.T) {
	...
	actions, syncErr := getSyncActionsWithProgress(runID, srcDir, exclusions, dstDir, true, 0, false)
